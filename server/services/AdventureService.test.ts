...
describe('AdventureService', () => {
  ...
  describe('addCreatureToAdventure', () => {
    it('should add a creature to an adventure', async () => {
      ...
      const mockCreature = Creature.build({
        ...
        adventureid: '68c8bd92-04ff-4359-9856-8d2d6b02b69b',
        name: 'Playful Kitten 2',
        image: '/token-playful-kitten2.png',
        metadata: JSON.stringify([
          {
            name: 'Type',
            type: 'string',
      ...
      jest.spyOn(AdventureCreature, 'build').mockImplementation(() => {
        return {
          save: jest.fn()
        };
      });
      ...
      const result: AdventureResponse = await AdventureService.addCreatureToAdventure('1', '1');
    ...
    it('should throw when adventure id is not present', () => {
      expect(async () => {
        await AdventureService.addCreatureToAdventure(null, '1');
      }).rejects.toThrow(MissingArgumentException);
    });

    it('should throw when creature id is not present', () => {
      expect(async () => {
        await AdventureService.addCreatureToAdventure('1', null);
      }).rejects.toThrow(MissingArgumentException);
    });
    ...
    it('should throw an error when an adventure is not found', () => {
      ...
      expect(async () => {
        await AdventureService.addCreatureToAdventure('1', '1');
  ...
  describe('addHandoutToAdventure', () => {
    it('should add an equipment item to an adventure', async () => {
      const mockAdventure = Adventure.build({
        id: '1',
        name: 'Foo Adventure',
      ...
      jest.spyOn(AdventureHandout, 'build').mockImplementation(() => {
        return mockAdventureHandout;
      });
      ...
      const result: AdventureResponse = await AdventureService.addHandoutToAdventure(request);
    ...
    it('should throw when request is not present', () => {
      expect(async () => {
        await AdventureService.addHandoutToAdventure(null);
      }).rejects.toThrow(MissingArgumentException);
    });

    it('should throw when request is not valid', () => {
      expect(async () => {
        const request = new AddAdventureHandoutRequest(
          '',
          '',
        ...
        await AdventureService.addHandoutToAdventure(request);
      }).rejects.toThrow(AdventureHandoutRequestMalformedException);
    });
