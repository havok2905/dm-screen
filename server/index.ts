...
const AdventureCreature = sequelize.define('adventurecreature', {
  id: {
    type: DataTypes.TEXT,
    primaryKey: true
  },
...
const AdventureHandout = sequelize.define('adventurehandout', {
  id: {
    type: DataTypes.TEXT,
    primaryKey: true
  },
...
const AdventureItem = sequelize.define('adventureitem', {
  id: {
    type: DataTypes.TEXT,
    primaryKey: true
  },
...
const database = new sqlite3.Database(DBSOURCE, (error) => {
    if (error) {
      ...
      database.run(createAdventureSQL, () => {
        database.run(createAdventureCreatureSQL, () => {
          database.run(createAdventureHandoutSQL, () => {
            database.run(createAdventureItemSQL, () => {
              seedData();
            })
          })
        })
...
app.get('/adventure/:id', async (request, response) => {
  ...
  response.json({
    ...
    creatures: adventureCreatures.map((c: any) => {
      return {
        ...c.dataValues,
        metadata: JSON.parse(c.dataValues.metadata)
      };
    }),
    handouts: adventureHandouts.map((h: any) => h.dataValues),
    items: adventureItems.map((i: any) => {
      return {
        ...i.dataValues,
